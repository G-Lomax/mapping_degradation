...
sample_points <- sample_points_raw %>%
  ...
  select(-allRangelands, -igbp, -shrubland, -grassland, -trees, -ID) %>%
...
sample_points_derived %>%
  mutate(woody_cat = cut_width(wriTCFrac, 0.2, boundary = 0)) %>%
...
set_threads(lrn_ranger_tuned_spt, 4)
set_threads(lrn_ranger_tuned_nspt, 4)
...
using the model tuned with LLTO-CV.

```{r rpi}

sample_points_rpi <- data_with_quantiles_spt %>%
  mutate(rpi = GPP / quantile_pred) %>%
  bind_cols(sample_points_derived %>% select(id, year, geometry, precipitation))
...
pixel_ids_df <- sample_points_rpi %>%
  filter(wriTCFrac < 0.05) %>%
  select(id)

pixel_ids <- unique(pixel_ids_df$id)
set.seed(12)
random_ids <- sample(pixel_ids, 12)

rpi_sample <- sample_points_rpi %>%
  filter(id %in% random_ids)

ggplot(rpi_sample, aes(x = year, y = rpi)) +
  geom_line() +
...
# )
# 
# p1 <- ggplot(rpi_sample, aes(x = year, y = mean_ppt * ppt_anomaly)) +
#   geom_col(fill = "#203864", alpha = 0.75) +
#   theme_classic() +
#   labs(x = "Year", y = "Annual rainfall") +
#   transparent 
# 
# p2 <- ggplot(rpi_sample, aes(x = year, y = GPP)) +
#   geom_col(fill = "#4C5128", alpha = 0.75) +
#   theme_classic() +
#   labs(x = "Year", y = "Vegetation productivity") +
#   transparent 
# 
# p3 <- ggplot(rpi_sample, aes(x = year, y = rpi)) +
#   geom_col(fill = "#843C0C", alpha = 0.75) +
#   theme_classic() +
#   labs(x = "Year", y = "Vegetation condition") +
#   geom_hline(yintercept = 1, alpha = 0.75) +
#   transparent
# 
# p1;p2;p3
# 
# target <- random_ids[6]
# 
# ggsave(here("results", "figures", "p1.png"), p1, bg = "transparent",
...
```{r autocorrelation_and_trends}

rpi_ac <- sample_points_rpi %>%
  group_by(id) %>%
  arrange(year, .by_group = TRUE) %>%
  summarise(ac = acf(rpi, lag.max = 2, plot = FALSE, demean = TRUE)$acf[2],
            geometry = first(geometry),
            map = mean(mean_ppt),
            mat = mean(tMean))

hist(rpi_ac$ac, breaks = 100)
mean(rpi_ac$ac, na.rm = TRUE)

# Basically no average autocorrelation! What about spatially or by variable?
tm_shape(rpi_ac %>% st_as_sf(crs = 4326)) + tm_dots(col = "ac", palette = "RdBu") +
  tm_shape(ke_tz) + tm_borders()

ggplot(rpi_ac, aes(x = map, y = ac)) +
  geom_point(size = 0.1, alpha = 0.1) +
  geom_smooth(method = "lm") +
  theme_bw()


# Obviously some spatial clustering, but no clear relationship to ppt or temp
# Hard to know what that means

rpi_trends <- sample_points_rpi %>%
  group_by(id, geometry) %>%
  filter(n() == 19) %>%
  arrange(year, .by_group = TRUE) %>%
  nest() %>%
  mutate(lm = map(data, function(df) {lm(rpi ~ year, data = df)}),
         kendall = map(data, function(df) {Kendall::MannKendall(df$rpi)}),
         theil_sen = map(data, function(df) {deming::theilsen(rpi ~ year, data = df)}),
         map = map(data, function(df) {mean(df$mean_ppt)}),
         mat = map(data, function(df) {mean(df$tMean)})) %>%
  select(-data)

rpi_trends_metrics <- rpi_trends %>%
  mutate(tidy_lm = map(lm, broom::tidy),
         lm_slope = map(tidy_lm, function(df) {df$estimate[2]}) %>% unlist(),
         lm_p_value = map(tidy_lm, function(df) {df$p.value[2]}) %>% unlist(),
         kendall_tau = map(kendall, function(ken) {ken$tau}) %>% unlist(),
         kendall_p_value = map(kendall, function(ken) {ken$sl}) %>% unlist(),
         theilsen_slope = map(theil_sen, function(mod) {mod$coefficients[2]}) %>% unlist()) %>%
  select(-lm, -tidy_lm, -kendall, -theil_sen) %>%
  ungroup()
         

head(rpi_trends_metrics)

hist(rpi_trends_metrics$kendall_tau, breaks = 100)

ggplot(rpi_trends_metrics, aes(x = map, y = theilsen_slope)) +
  geom_point(size = 0.2, alpha = 0.2) +
  geom_smooth(method = "lm") +
  theme_bw()

tm_shape(rpi_trends_metrics %>% st_as_sf(crs = 4326)) + tm_dots(col = "theilsen_slope", palette = "RdBu", style = "cont") +
  tm_shape(ke_tz) + tm_borders()
