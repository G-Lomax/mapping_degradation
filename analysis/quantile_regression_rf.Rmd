...
When defining the learner, we initially choose 
...
lrn_ranger_tuned_spt <- lrn(
  ...
  min.node.size = to_tune(p_int(10, 10000, logscale = TRUE)),
...
The random CV model appears to have significantly higher R-squared and lower
error than the one with spatio-temporal LOO-CV. Looking at the plot, a linear
array of points along the 1:1 line is clearly visible, which to me suggests
overfitting is very probable. Perhaps I should re-run the models with nested
cross-validation to get an independent assessment of fitting ability.
...
sample_points_rpi <- data_with_quantiles_spt %>%
  ...
  bind_cols(sample_points_clean %>% select(id, year, woody, geometry, precipitation))
...
tm_shape(ke_tz) + tm_borders() +
  tm_shape(sample_points_rpi %>% filter(year == 2006) %>% st_as_sf(crs = 4326)) +
  tm_dots(col = "rpi", palette = "PiYG", style = "cont", midpoint = 1)
```

Observations:
- The annual variations remain in the boxplots, which implies that additional
driving factors (e.g., precipitation timing) are needed in the model
- There remains a weak positive correlation between mean_ppt and RPI, which I
think reflects the tighter grouping (fewer very low values) at high mean PPT.
