---
title: "Tanzania People and Wildlife: Data Exploration and MSAVI Time Series"
author: "Guy Lomax"
date: '2023-03-31'
output:
  html_document:
...
In this notebook, I generate smooth, high-resolution MSAVI time series for
...
continuous MSAVI time series that provide detailed insight into the
...
## Combine with Sentinel-2 MSAVI time series data from Google Earth Engine

The GPS coordinates of the transect locations were used to extract local
MSAVI (Normalised Difference Vegetation Index) and MSAVI2 (Modified
...
visualise the MSAVI time series.

```{r full_ts, cache = T}

# Load and clean MSAVI time series data from GEE
...
# Merge double readings (more than one value in a day) by taking higher msavi
...
MSAVI tends to change smoothly, without abrupt spikes or drops. (b) Cloud
cover and other disturbances tend to reduce MSAVI values compared to
their true values.
...
tpw_all_int <- tpw_ts_unique %>%
  group_by(plot_id) %>%
  arrange(imgDate) %>%
  mutate(msavi_int = na_interpolation(msavi),
         msavi_spike = (lead(msavi_int) - msavi_int > 0.1),
         msavi_drop = (lag(msavi_int) - msavi_int > 0.1),
         msavi_int = na_if(msavi_int, ((msavi_spike | msavi_drop) * msavi)),
         msavi_int = na_interpolation(msavi_int)) %>%
  select(-msavi_spike, -msavi_drop)
...
#   group_by(plot_id) %>%
#   mutate(opt = find_least_error(msavi_int))

tpw_sg1 <- tpw_all_int %>%
  group_by(plot_id) %>%
  mutate(msavi_filtered = sgolayfilt(msavi_int, p = 3, n = 25))

# Assign weights to MSAVI values in original (interpolated) series based on
# whether they fall above or below the trend curve for that TS.

tpw_weights <- tpw_sg1 %>%
  mutate(dist = msavi_filtered - msavi_int,
         max_dist = max(dist),
         weight = ifelse(dist > 0, 1 - (dist / max_dist), 1))

# Iterative approach to upper msavi envelope
# Generate new time series by replacing "noisy" MSAVI values with filtered ones
# Second, shorter-period SG filter

iterative_fit <- function(msavi_1, msavi_0, weight) {
  
  initial_fit <- 100
  new_fit <- sum(abs(msavi_1 - msavi_0) * weight)
  msavi_new <- msavi_1

  while(initial_fit > new_fit) {
    initial_fit <- new_fit
    
    msavi_new <- ifelse(msavi_new >= msavi_0, msavi_new, msavi_0) %>%
    sgolayfilt(p = 4, n = 15)
  
  new_fit <- sum(abs(msavi_new - msavi_0) * weight)
  }
  msavi_new
}
...
tpw_final <- tpw_weights %>%
  mutate(msavi_new = iterative_fit(msavi_filtered, msavi_int, weight)) %>%
  select(-dist, -max_dist, -weight, -msavi_int, -msavi_filtered)

# tpw_final %>%
#   filter(plot_id %in% sample_plots) %>%
#   pivot_longer(cols = c("msavi_int", "msavi_filtered", "msavi_new"),
...
tpw_final %>%
  ...
  geom_line(aes(y = msavi_new), colour = "green", size = 1) +
  geom_point(aes(y = msavi), colour = "grey40", size = 0.5) +
  # geom_line(aes(y = msavi), colour = "grey40") +
  facet_wrap(~plot_id) +
  theme_clean() +
  # scale_x_date(breaks = "6 months", minor_breaks = "3 month") +
  ylim(0, 1) +
...
tpw_metrics <- tpw_hyear %>%
  group_by(plot_id) %>%
  mutate(min_msavi = min(msavi_new),
         msavi_pc10 = quantile(msavi_new, 0.1),
         growing_season = msavi_new > msavi_pc10) %>%
  group_by(plot_id, hyear) %>%
  summarise(min_msavi = first(min_msavi),
            seasonal_min_msavi = min(msavi_new),
            peak_msavi = max(msavi_new) - min_msavi,
            msavi_auc = sum(msavi_new * growing_season) - 
              (sum(growing_season) * min_msavi),
...
tpw_hyear %>%
  ...
  ggplot(aes(x = imgDate)) +
  geom_line(aes(y = msavi_new), colour = "green") +
  geom_point(aes(y = msavi), colour = "grey", size = 0.2) +
  geom_col(aes(y = ppt_scaled), alpha = 0.5) +
  facet_wrap(~plot_id) +
  ylim(0, 1) +
  theme_clean() +
  labs(x = "Date", y = "MSAVI",
       title = "MSAVI relative to rainfall timing and amount",
       subtitle = "Columns represent dates of rainfall")

tpw_metrics %>%
  filter(plot_id %in% sample_plots) %>%
  ggplot(aes(x = season_ppt, y = msavi_auc, colour = plot_id, shape = plot_id)) +
  geom_point(size = 2) +
  theme_clean() +
  labs(x = "Seasonal rainfall (mm)",
       y = "Integrated seasonal MSAVI",
       colour = "Plot",
       shape = "Plot")
...
MSAVI at the start or end of the season. More rapid recovery and greater
...
the MSAVI profile for each year.
...
tpw_derivatives <- tpw_hyear %>%
  select(plot_id, hyear, imgDate, msavi, msavi_new) %>%
  group_by(plot_id) %>%
  arrange(imgDate) %>%
  mutate(msavi_smooth = smooth.spline(imgDate, msavi_new)$y) %>%
  mutate(msavi_1d = (lag(msavi_new) - lead(msavi_new)) / as.numeric(lag(imgDate) - lead(imgDate)),
         msavi_1ds = (lag(msavi_smooth) - lead(msavi_smooth)) / as.numeric(lag(imgDate) - lead(imgDate)),
         msavi_2d = (lag(msavi_1d) - lead(msavi_1d)) / as.numeric(lag(imgDate) - lead(imgDate)),
         msavi_2ds = (lag(msavi_1ds) - lead(msavi_1ds)) / as.numeric(lag(imgDate) - lead(imgDate)))
...
tpw_max_min <- tpw_derivatives %>%
  mutate(is_max = (msavi_1ds * lag(msavi_1ds) < 0) & (msavi_2ds < 0),
         is_min = (msavi_1ds * lag(msavi_1ds) < 0) & (msavi_2ds > 0))

# For each hydrological year, identify the greatest increase and decrease
# between subsequent maxima/minima

# 1. Create data frame of only max and min rows (including MSAVI, imgDate and
#    plot_id) arranged by imgDate
# 2. Calculate MSAVI differences between rows
...
tpw_events <- tpw_max_min %>%
  ...
  select(plot_id, imgDate, hyear, msavi_new, msavi_smooth, is_max, is_min) %>%
  mutate(lead_diff = lead(msavi_new) - msavi_new,
         lag_diff = lag(msavi_new) - msavi_new,
...
tpw_fitting <- tpw_hyear %>%
  ...
  group_by(plot_id, hyear) %>%
  mutate(msavi_scaled = msavi_new - min(msavi_new)) %>%
...
tpw_growth_model <- tpw_fitting %>%
  filter(growth_leg) %>%
  summarise(mod = list(growth_fit(msavi_scaled, imgDate, "logistic")))
t2 <- Sys.time()
...
tpw_decay_model <- tpw_fitting %>%
  filter(decay_leg) %>%
  summarise(mod = list(decay_fit(msavi_scaled, imgDate, "logistic")))
t2 <- Sys.time()
...
tpw_fitted <- tpw_fitting %>%
  ungroup() %>%
  full_join(growth_fitted, by = c("plot_id", "hyear", "msavi_scaled" = "ts")) %>%
  full_join(decay_fitted, by = c("plot_id", "hyear", "msavi_scaled" = "ts")) %>%
