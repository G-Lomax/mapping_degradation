...
writeRaster(rue_mean, here("data", "processed", "raster", "rue_mean.tif"), overwrite = TRUE)
writeRaster(rue_trend, here("data", "processed", "raster", "rue_trend.tif"), overwrite = TRUE)
...
fit_restrend <- function(gpp_rast, ppt_rast, tMean_rast) {
  ...
  tMean_values <- as.data.frame(tMean_rast, cells = TRUE, xy = FALSE)
...
restrend_results <- app(restrend_rast, function(x) {
  ...
  if (any(is.na(x))) {
    ...
    df <- data.table(year = seq_len(length(resids)), resids = resids)
