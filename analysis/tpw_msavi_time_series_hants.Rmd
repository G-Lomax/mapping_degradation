...
tpw_all_int <- tpw_ts_unique %>%
  ...
  mutate(msavi_int = na_interpolation(msavi),
         msavi_spike = (lead(msavi_int) - msavi_int > 0.2),
         msavi_drop = (lag(msavi_int) - msavi_int > 0.2),
         msavi_int = na_if(msavi_int, ((msavi_spike | msavi_drop) * msavi)),
         msavi_int = na_interpolation(msavi_int)) %>%
  select(-msavi_spike, -msavi_drop)
         

tpw_hants <- tpw_all_int %>%
  mutate(msavi_hants = haRmonics(
    y = msavi_int,
    method = "hants",
    # lenPeriod = 73,
    numFreq = 24,
...
tpw_hants %>%
  ...
  geom_line(aes(y = msavi_hants), colour = "green", size = 1) +
