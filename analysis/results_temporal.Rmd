...
kendall_tau_all <- c(rue_kendall[[1]], restrend_kendall[[1]], rpi_kendall[[1]])
names(kendall_tau_all) <- c("RUE", "RESTREND", "RPI")
kendall_tau_df <- as.data.frame(kendall_tau_all, na.rm = T, cell = T)

kendall_tau_plot <- kendall_tau_df %>%
  pivot_longer(cols = -"cell", names_to = "metric", values_to = "kendall_tau") %>%
  mutate(metric = ordered(metric, levels = c("RUE", "RESTREND", "RPI"))) %>%
  ggplot(aes(x = kendall_tau, fill = metric)) +
  geom_density(colour = "grey30", alpha = 0.6) +
  geom_vline(xintercept = 0) +
  scale_fill_brewer(palette = "Set2") +
  theme_bw() +
  labs(x = "Kendall Tau",
...
# Mean, median and SD values per index

kendall_metrics <- kendall_tau_df %>%
  pivot_longer(cols = -"cell", names_to = "metric", values_to = "kendall_tau") %>%
  group_by(metric) %>%
  summarise(mean = mean(kendall_tau),
            median = median(kendall_tau),
            sd = sd(kendall_tau))
...
rpi_rue_plot <- ggplot(trend_sample_subset, aes(x = slope, y = rue_slope)) +
  geom_point(size = 0.5) +
  xlim(-0.03, 0.03) +
  ylim(-0.2, 0.2) +
...
rpi_restrend_plot <- ggplot(trend_sample_subset, aes(x = slope, y = resid_slope)) +
  geom_point(size = 0.5) +
  xlim(-0.03, 0.03) +
  # ylim(-0.2, 0.2) +
  geom_hline(yintercept = 0) +
